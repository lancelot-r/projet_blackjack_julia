...
Jeudi 17/10 : On a une ébauche (bugué) de notebook Pluto pour le jeu. On essaye donc de réparer ça parce que les boutons ne font pas ce que l'on veut. Possibilités : passer en markdown, nettoyer le code pour séparer les calculs et l'affichage en faisant des fonctions. On veut aussi implémenter le fait d'afficher des images dans le notebook.

# Trucs à faire :
- graphicstrial.jl -> Passer les textes en Markdown plutôt que des prints.
- graphicstrial.jl -> Séparer les calculs et l'affichage (faire des fonctions).
...
Gary :
- Faire un projet pour benchmark R et Julia et Python
- Tester avec PythonCall dans Julia pour les tester en même temps pour différentes valeurs de n.
Lancelot :
